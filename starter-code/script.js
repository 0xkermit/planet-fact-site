...
hamburger.addEventListener("click", () => {
  if (!isMenuOpen) {
    ...
    isMenuOpen = true;
    planetInfo.classList.add("isHidden");
  } else {
...
planetTab.forEach((tab) => {
  tab.addEventListener("click", () => {
    const selectedTab = document.querySelector(".selected");
    if (tab.id === "overview") {
      type = "overview";
      changingContent(currentPlanet, type);
    } else if (tab.id === "surface-geoglogy") {
      type = "geology";
      changingContent(currentPlanet, type);
    } else if (tab.id === "internal-structure") {
      type = "structure";
      changingContent(currentPlanet, type);
    }
    if (selectedTab) {
...
planetSelection.forEach((planetClicked, i) => {
  planetClicked.addEventListener("click", () => {
    ...
    planetClicked.classList.add("selectedPlanet");
    planetClicked.style.borderBottomColor = data[i].color;
...
overviewButton.addEventListener("click", () => {
  type = "overview";
  internalButton.classList.remove("type-selector-selected");
  geologyButton.classList.remove("type-selector-selected");
  overviewButton.classList.add("type-selector-selected");
  changingContent(currentPlanet, type);
});

internalButton.addEventListener("click", () => {
  type = "structure";
  overviewButton.classList.remove("type-selector-selected");
  geologyButton.classList.remove("type-selector-selected");
  internalButton.classList.add("type-selector-selected");
  changingContent(currentPlanet, type);
});
geologyButton.addEventListener("click", () => {
  type = "geology";
  overviewButton.classList.remove("type-selector-selected");
  geologyButton.classList.add("type-selector-selected");
  internalButton.classList.remove("type-selector-selected");
  changingContent(currentPlanet, type);
});

function changingContent(planetId, type) {
  if (type === "overview") {
    planetImage.src = data[planetId].images.planet;
    planetContent.innerText = data[planetId].overview.content;
    planetSource.href = data[planetId].overview.source;
  } else if (type === "structure") {
    planetImage.src = data[planetId].images.internal;
    planetContent.innerText = data[planetId].structure.content;
    planetSource.href = data[planetId].structure.source;
  } else if (type === "geology") {
    planetImage.src = data[planetId].images.geology;
    planetContent.innerText = data[planetId].geology.content;
    planetSource.href = data[planetId].geology.source;
  }
